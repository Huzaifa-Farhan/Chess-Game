...
function insertImage() {
    document.querySelectorAll('.box').forEach(image => {
        if (image.innerText.length !== 0) {
            let imagePath = 'images/' + image.innerText; // Set the path to the image folder
            if (image.innerText === 'W_Pawn' || image.innerText === 'B_Pawn') {
                ...
                image.style.cursor = 'pointer'; // Change cursor style
            } else {
                // General class for other pieces
...
insertImage(); // Call the function to execute
...
function coloring() {
    ...
    color.forEach(color => {
        ...
        if (a % 2 == 0) {
            color.style.backgroundColor = 'rgb(112,162,163)'
        }
        if (a % 2 !== 0) {
            color.style.backgroundColor = 'rgb(177,228,185)'
        }
    })
}
coloring()
